...
2026-09-13: Keep catalogue search local and status reads authenticated and non-mutating; preserve six-hour cross-sync cooldown and 20-second local extension heartbeat, add no polling, and verify cancellation/retry behavior with synthetic Chromium fixtures. Exact HEAD counts avoid capped membership payloads but still require database counting work.

2026-09-14: Keep public web files byte-identical while moving their delivery ahead of the Python catch-all. Preserve production security headers and revalidate unversioned browser assets. Repair the extension's documented six-hour cadence and migrate retained alarms on lifecycle events without triggering a sync. Expose the existing on/off control, keep manual full sync and all stored records. Seven real worker tests failed against the baseline and pass after the correction. Local validation passes 89 Python tests, 11 Node tests, 13 Chromium cases and a real popup dispatch check; a local Vercel build and independent route/alarm review also pass. Hosted release and measured savings remain pending.
